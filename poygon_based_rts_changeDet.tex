...
Permafrost occurs \citep{zhang1999statistics}.

Evolution of retrogressive thaw slumps is poor quantified in most permafrost areas.\\\\


Contribution of this works: (1) provide an automatic method for calculating the retreat distance (no matter the RTS boundaries are from manual delineation or automatic mapping). (2) What’s the IOU value of mapping polygons are required for automatic change detection? For polygon-based change detection, we also can calculate F1 score, but it rely on the accuracies of mapping results. \\\\
