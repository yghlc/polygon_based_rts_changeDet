...
However, RTS distribution and evolution are poorly quantified because most of them occur in remote and inaccessible areas. % due to the challenges in accurately mapping them from remote sensing images. 
...
Our method reveals that among manually-delineated 342 RTSs in the Beiluhe region, 83\% and 76\% of them expanded from 2017 to 2018 and 2018 to 2019, respectively.
...
For 2018 to 2019 the average and maximum expansion areas and retreat distances  are 0.22 ha, 2.53 ha, 25.0 m, and 212 m, respectively.
The results show that the method can quantify RTS development automatically on multi-temporal images \replaced{but may miss some small and subtle RTSs}{ and can be potentially applied to larger areas}. 
...
% some background
Permafrost is the ground that remains below 0$^\circ$C for at least two consecutive years and is warming at the global scale \citep{biskaborn2019permafrost}. 
...
\begin{figure} 
	...
	% and test area 
	on the Tibetan Plateau. (b) to (d) are Planet images taken in 2017, 2018, and 2019 of an RTS whose central location is 92.912$^\circ$ E, 34.848$^\circ$ N. The white arrows point to the headwall which retreated upslope (also shown in the video in the Supplementary Materials). The white rectangle in (d) is the extent of Fig. \ref{fig_rts_expanding} } % In (b), the yellow arrow indicates the upslope direction.
...
\begin{figure}
	...
	\caption{Flowchart of automatic quantification of RTS development on multi-temporal images. The automated process includes three major steps: (1) pre-processing of multi-temporal Planet images, (2) automatic delineation, and (3) automatic quantification of RTS expansion.}
	\label{fig_flowchart}
\end{figure}

 As illustrated in Fig. \ref{fig_flowchart}, the major procedures of the automatic method for quantifying RTS evolution include pre-processing of Planet images, automatic delineation of RTSs, and automatic quantification of their expansion. 
...
we manually delineated RTS boundaries on Planet images taken in all the three years as accurately as possible. 
...
Depending on the experiments (section \ref{sec_prepare_training}), parts of these RTS boundaries were used for training our automatic delineation method, and all of them would be considered as ground truth\deleted{s} for validation. 
...
The training polygons consist of parts of \added{or all} the \replaced{manually-drawn polygons}{ground truths} (i.e., positive training polygons) and polygons covering non-RTS regions (termed as negative training polygons) in different years.
...
We chose all the \replaced{manually-drawn polygons}{ ground truths} as well as negative training polygons for training. 
To assess the capability and generalization of DeepLabv3+ when applied to multi-temporal Planet images, we conducted experiments using (1) \replaced{manually-drawn polygons}{ ground truths} of 2017 and 2018 as well as 78 negative training polygons for training and (2) all \replaced{manually-drawn polygons}{ ground truths} but without negative training polygons.
...
We did not conduct more experiments (e.g., k-fold cross-validation) using different portions of \replaced{manually-drawn polygons}{ ground truths} because these tend to obtain similar accuracies as proved by \cite{huang2020using}.


For the training polygons and Planet images in each year, we extracted sub-images with a buffer area of 300 m. 
These sub-images are image subsets covered by the training polygons and their surroundings.
Then we subdivided all sub-images into patches by setting a patch size as 480 $\times$ 480 pixels with an overlap of 160 pixels to meet the input requirement of DeepLabv3+. 
...
As shown in Fig. \ref{fig_rts_expanding}, an active RTS can expand toward upslope annually; even for a non-active RTS in our study area, the possibility of re-vegetation and shrinking of its extent is very low due to the short period of time (three years).
...
which allows us to keep the RTSs that emerged in 2018 but remove the new ones in 2019.
...
\begin{figure} 
	...
	\caption{RTS polygons (a) representing their boundaries from 2017 to 2019 and the union (b) of these polygons. The background is a subset of 2019 Planet images marked by the white rectangle in Fig. \ref{fig_multi_rts_image_studyarea}(d), and the RTS is the same as the one shown in Fig. \ref{fig_multi_rts_image_studyarea}. The bright areas in the upper parts of the image are traces of mass movements, not part of the RTS.}
...
Firstly, by comparing the mapped polygons and the corresponding ground truth\deleted{s}, we calculated IOU values of each mapped polygon. 
...
AP values are the areas under precision-recall curves which is an effective metric for evaluating the algorithm performance because it illustrates the relationship between precision and recall for different IOU thresholds \citep{huang2020using}. 
...
\subsection{Automatic quantification of RTS expansion}
\label{sec_auto_rts_develop}

To automatically quantify RTS expansion with the input of RTS boundaries, we proposed a method to detect changes of RTS boundaries and to calculate their annual retreat distances.
...
The distribution of mapped polygons for the entire Beiluhe region is shown in Fig. \ref{fig_mapping_results}a, and magnifications of three locations are shown in Fig. \ref{fig_mapping_results}b--d. 
Compared with the ground truth\deleted{s} presented in Fig. S1 in the Supplementary Materials,  around 90 RTSs are missed in the mapped polygons such as the three within the yellow rectangle in Fig. \ref{fig_mapping_results}b.
As shown in Fig. \ref{fig_mapping_results}c and d, the true positives are close to the ground truth\deleted{s}, that is, the automatically delineated ones are similar to the ones from manual delineation. 
...
The experiment excluding the 2019 image during training demonstrates the good generalization of our trained model for locating RTSs but slightly worse for delineating boundaries. 
...
The number of TP in the mapped polygons of 2019 is less than the ones in other years by around 50, indicating relatively lower delineating accuracies (boundaries are inaccurate) in the test image (excluded during training). 
After applying the RTS dynamic features, all F1 scores remain at similar values (Table S2), but both TP and FP values reduce dramatically, from $\sim$290 to $\sim$190 and from $\sim$200 to $\sim$30, owing to the relatively lower delineating accuracies on the 2019 image.
...
This is because excluding negative training polygons tends to produce more false positives but they \replaced{seldom}{ seldomly} can meet the criteria of the RTS dynamic feature.  
...
Regarding the F1 scores (Table \ref{table_accuracy_f1}) after applying RTS dynamic features, the difference between the results in \cite{huang2020using} and in this paper is around 0.05, while the difference of the AP values is around 0.1.
...
One possible reason is related to the variation of features over time, which may increase the bias between training and test data. 
...
\subsubsection{Area of RTS expansion based on manual delineation}
...
\subsubsection{RTS retreat distance\deleted{ based on manual delineation}}
\label{sec_rts_retreat_distance}

%retreat distance from 2017 to 2019. 
%add standard variation for retreat distance.  done
The average retreat distances of RTSs from 2017 to 2018 and 2018 to 2019 were 20.1 m and 20.9 m (Fig. \ref{fig_rts_change_area_reDis}c and d)\added{ based on manually-drawn boundaries}, respectively. 
...
As shown in Fig. S6, the mean differences are $-1.3$ m and $-4.1$ m, 17.9 m and 14.4 m (2017 vs 2018 and 2018 vs 2019), and 13.4 m and 9.7 m for the one based on the diameter of the maximum medical circles, the direction of the maximum elevation difference, and across the centroid of old polygons, respectively. 
...
From 2017 to 2018, the percentage of RTSs having standard deviations smaller than 10 m is 60\%, while for 2018 to 2019, it is 59\%, demonstrating a fairly good agreement among the three measurements.
...
However, the total numbers of expanding RTSs are 236 and 233 from 2017 to 2018 and 2018 to 2019, respectively, which are smaller than the ones (284 and 259) based on manual delineation, due to the false negatives in the automatic delineation results.
\added{The total expansion areas of all RTSs from automatic delineation are 59.9 ha (2017 vs 2018) and 56.4 ha (2018 vs 2019), slightly larger than manual delineation ones (58.1 ha and 55.7 ha).}
...
The possible reasons include (1) multi-temporal images tend to have different acquisition conditions, which may increase the bias between the training and test data and lead to lower prediction performance \citep{tuia2016domain}, and (2) changes of RTSs appearance and the surrounding over time increase the difficulty of delineating them.
...
Applying the RTS dynamic assumptions can significantly reduce the number of false positives (section \ref{sec_improve_using_multi_images}), but it also removes some correctly mapped polygons with relatively lower delineation accuracy and a new RTS (in or after 2019). 
...
Moreover, how to define retreat distances of RTSs when their expanding areas are irregular or elongated is still debatable as the distances can vary significantly across the scarp.
...
However, one-year model could improve the accuracies but cannot guarantee accurate RTS boundaries when applying to a test region \citep{huang2020using}.
We may also use domain adaptation techniques to reduce the effects introduced by the bias between training and test data \citep{tuia2016domain}.
...
Moreover, these criteria need to be adjusted if RTSs stabilize and revegetate in a longer (e.g., decadal) time scale.
Because the accuracy of automatic delineation is critical but is challenging to achieve, in the future, we may consider using pixel-based change detection to obtain the expanding area directly from multi-temporal images \citep{huang2020detecting}.
...
\label{sec_diss_retreat_distance}
The mean values of expansion areas and retreat distances could be slightly overestimated due to the limitation of image resolution. 
...
There remain many questions related to the retreat areas and distances such as their uncertainties for each RTS, spatial and temporal pattern of RTS expansion at longer (e.g., decadal) time scales.
In the future, we will conduct field measurements as ground truth\deleted{s} to validate the retreat areas and distances and calculate the corresponding uncertainties.
...
Experiments show that the results from the automatic delineation of RTS boundaries on multi-temporal images are close to those obtained from manual delineation for most RTSs, indicating the good performance of DeepLabv3+ not only for identifying their locations but also for accurately delineating their boundaries. 
...
The whole process is automatic and can be used as an effective and efficient tool \replaced{to facilitate the quantification of }{ for quantifying} RTS development in large areas. 

Most of the RTSs in the Beiluhe region expanded from 2017 to 2019 with significant expanding areas and retreat distances. 
Among the 342 RTSs delineated manually, 83\% of them expanded from 2017 to 2018 and 76\% from 2018 to 2019, with an average expanding area of 0.20 ha and 0.22 ha, respectively.
