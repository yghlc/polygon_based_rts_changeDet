...
%TODO: this table only list the images for Beiluhe study area, not includes the test Area.
\begin{table}[ht]
\footnotesize
...
\subsection{Mapping resulting using multi-temporal images}
...
% results from k-fold cross-validation (mention it, then put into supplementary files)

%compared to the one with negative training polygons?

%Improvement by applying time IOU and occurrence


% compared with the results with negative training polygons
Compared with the RSE 2020 paper, the AP values and F1 scores are lower, indicating the challenging of mapping RTSs in different years. 
