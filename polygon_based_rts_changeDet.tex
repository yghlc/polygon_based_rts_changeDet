...
%TODO: 0.3 ha, should I use ha or m^2
...
%TODO: this part (retreat distance) needs to be rewrite 
...
The distribution of mapped polygons for the entire Beiluhe region is shown in Fig. \ref{fig_mapping_results}a, amplifications of three locations are shows in Fig. \ref{fig_mapping_results}a, b, and c. 
...
Regarding the F1 scores (Table \ref{table_accuracy_f1}) after applying RTS dynamic features, their difference are around 0.17, while the difference of AP values are around 0.2.
...
In Beiluhe region, most RTS are expanding from 2017 to 2019 with average expanding area of 0.21 ha from 2017 to 2018 and 0.23 ha from 2018 to 2019 as shown in Fig. \ref{fig_rts_change_area_reDis}a and b.
From 2017 to 2018, 219 RTSs are expanding, among them, 77 RTSs with expanding area greater than 0.2 ha; while from 2018 to 2019, 189 RTSs are expanding, and 66 of them with expanding area greater than 0.2 ha.
...
\label{sec_rts_retreat_distance}
 ...
 Retreat distance (the data from )

Retreat distance from other measurement. 
...
A new emergent RTS was missed. 

If a few negative training polygons are available, including them for training can help improve the mapping results. 

Limitations

% compared with the results with negative training polygons
Compared with the mapped polygons on images taken in May 2018 presented by \cite{huang2020using}, the results in this paper are worse.
Less true positive, more false positive and false negatives. 
 the AP values and F1 scores are lower, indicating the challenging of mapping RTSs in different years. 

Mapping using multi-temporal images are more challenging. 
